 ...
 */
public class BTLSM_V2 extends SearchAlgorithm implements TechnicalInformationHandler {
...
//	private final boolean m_bInitAsNaiveBayes = false; 
	 ...
	 * default constructor
	 */
	public BTLSM_V2(int predictors, int max_parents, int max_children) {
	 ...
	 * default constructor
	 */
	public BTLSM_V2(int predictors, int max_parents, int max_children, int pess) {
		setExpectedParentsOfTarget(predictors);
		setMaxNrOfParents(max_parents);
		setMaxNrOfChildren(max_children);
	 ...
	 */
	//Ye added
		public BTLSM_V2(int predictors, int max_parents, int max_children, String sourceModelFile) throws Exception {
			setExpectedParentsOfTarget(predictors); 
			setMaxNrOfParents(max_parents);
			setMaxNrOfChildren(max_children);
...
//		System.out.println("BEFORE initVariables approach:");
//		System.out.println(print(m_BayesNet));
		
		initVariables2(); // targetNode is the last Node
		updateParents();
		...
		// initiate numberOfNodes, nodeDimension array, numberOfCases, initiate the size of the cases[][]
		//readCases();
		//initVariables();
		
	//	System.out.println("BEFORE initStructure approach:");
	//	System.out.println(print(m_BayesNet));
	//	m_BayesNet.initStructure();
		...
		weedoutWeakArcs();
	//	 updateBayesNet(); // update parent set of the bayesian network
		//EBMC1 Algorithm
		 int numberGraph=0;	 
...
//			}
	...
	public static BayesNet processSourceModel(BayesNet sourceModel){
		  BayesNet processedSourceModel = null;
   ...
   //     parents[node][0] = 0;
	 ...
	 * @throws Exception if something goes wrong
	 */
	public void buildStructure(String sourceModelFile, Instances targetInstances) throws Exception {
...
//		System.out.println(print(m_BayesNet));
		...
		initVariables(); // targetNode is the last Node
		...
		//EBMC1 Algorithm
		 int numberGraph=0;	 
		  while (hopeRemains && (numberOfChildren < maxChildren)) {
	            ...
	            updateBayesNet(); // update parent set of the bayesian network
	            m_BayesNet.estimateCPTs();
	            PrintWriter printout = new PrintWriter(new File("model" + numberGraph + ".xml"));
	            printout.println(m_BayesNet);
				printout.println(m_BayesNet.graph());
				printout.flush();
				printout.close();   
