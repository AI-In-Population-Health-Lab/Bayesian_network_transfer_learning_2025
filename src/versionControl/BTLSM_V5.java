 ...
 */
public class BTLSM_V5 extends SearchAlgorithm implements TechnicalInformationHandler {
...
//	private final boolean m_bInitAsNaiveBayes = false; 
	 ...
	 * default constructor
	 */
	public BTLSM_V5(int predictors, int max_parents, int max_children) {
	 ...
	 * default constructor
	 */
	public BTLSM_V5(int predictors, int max_parents, int max_children, int pess) {
		setExpectedParentsOfTarget(predictors);
		setMaxNrOfParents(max_parents);
		setMaxNrOfChildren(max_children);
	 ...
	 */
	//Ye added
	public BTLSM_V5(String sourceModelFile, int sourceSize, String scoreMethod) throws Exception {
		System.out.println("It was developed with " + sourceSize + " instances");
	...
	public void buildStructure(BayesNet bayesNet, Instances targetTrainingData) throws Exception {	
		 ...
		 * NOT DONE YET 2. remove not in markov Blanket: has not done yet. doMarkovBlanketCorrection(bayesNet, instances)	
		 * NOT DONE YET 3. move parentsToChildren: : has not done yet. 
		 * NOT DONE YET 4. inject target specific feature.
...
//		System.out.println(source_clean_BayesNet.graph());
		...
		//To do: inject target specific features and add dummy links between these features and the class (has not done yet)
...
//		System.out.println(m_BayesNet); //
	//	updateBayesNet(); // update parent set of the bayesian network based on parents[][]
		...
		printout.println(m_BayesNet.graph());
		printout.flush();
		printout.close();		
					
		writeModel(new File("00experiment/log/writeModel_test.txt"));
		
		/*Part 4. get Performance*/
		String testDataset = new String("C:/Users/yey5/Dropbox/YeEclipseThesis/Simulation/data/BrainTumor_True_Target_seed1120148509_size10000.arff");				
		...
		String trueTargetModel = new String("C:/Users/yey5/Dropbox/YeEclipseThesis/Simulation/model/BrainTumor_True_Target.bif");
...
//	private ArrayList<String> getParentConfigList(int iNode){
//		ArrayList<String> parentConfigList = new ArrayList<String>();
//		ParentSet parents = m_BayesNet.getParentSet(iNode);
//		int numParents = parents.getNrOfParents();
//		for (int iParent=0; iParent<numParents; iParent++){
//			int currentParent = parents.getParent(iParent);
//			
//		}
//		
//		return parentConfigList;
//	}
	...
	public void getCPTCombineModelData() throws Exception{
		...
		for (int iNode=0; iNode<estimators.length; iNode++){
			System.out.print("Node " + m_BayesNet.getNodeName(iNode)+ ":");
			...
			//adjustedPriorSampleSize*P(the node, the first parent, the second parent in prior network)
			Map<List<String>,Double> priorProbTable = getPesudoCountTable(iNode);
			...
			for (int jParentsConfig=0; jParentsConfig<nParentConfig; jParentsConfig++){
				...
				for (int iNodeValue=0; iNodeValue<nNodeValues; iNodeValue++){
					...
					theCount.addValue(data, thePesudoCount); //add pesudoCount
					//probs[jParentsConfig][iNodeValue] = theCount.getProbability(iNodeValue);
					//m_BayesNet.setDistribution(iNode,probs);
...
//	public double[] getPesudoCountsForOneParenetConfig(int iNode, int jParentConfig, Map<List<String>,Double> table){
//		ParentSet parents = m_BayesNet.getParentSet(iNode);
//		
//	}
	 ...
	 * @return
	 */
	public Map<List<String>,Double> getPesudoCountTable(int iNode){
...
//		}	
	...
	protected void writeModel(File outFile) throws IOException {
        deriveTargetCounts();
        BufferedWriter writer = null;
        try {
            writer = new BufferedWriter(new FileWriter(outFile));
            writer.write(String.format("%s\n", outFile.getName()));
            writer.write(String.format("%s\n", TimeUtility.formatter.print(new DateTime(System.currentTimeMillis()))));
            writer.write(String.format("%s\n", ""));
            writer.write(String.format("%d %d ", targetNode, nodeDimension[targetNode]));
            for (int targetValue = 1; targetValue <= nodeDimension[targetNode]; targetValue++)
                writer.write(String.format("%d ", targetCounts[targetValue]));
            writer.write(String.format("%d ", numberOfChildren));  // write number of children
            // write children
            for (int i = 1; i <= numberOfChildren; i++) {
                writer.write(String.format("%d ", children[i]));
            }
            // write parents of children
            for (int i = 1; i <= numberOfChildren; i++) {
                int child = children[i];
                writer.write(String.format("%d ", parents[child][0]));
                for (int j = 1;j <= parents[child][0]; j++) {
                    writer.write(String.format("%d ", parents[child][j]));
                }
            }
            for (int i = 1; i <= numberOfChildren; i++) {
                int c = fileCaseCache[i].countsTreePtr - 1;
                writer.write(String.format("%d ", c));
                for (int j = 1; j <= c; j++) {
                    writer.write(String.format("%d ", fileCaseCache[i].countsTree[j]));
                }
                c = fileCaseCache[i].countsPtr - 1;
                writer.write(String.format("%d ", c));
                for (int j = 1; j <= c; j++) {
                    writer.write(String.format("%d ", fileCaseCache[i].counts[j]));
                }
            }
        } finally {
            if (writer != null) {
	 ...
	 /**
     * Derive counts for the target node.
     */
    protected void deriveTargetCounts() {
        for (int targetValue = 1; targetValue <= nodeDimension[targetNode]; targetValue++) {
            targetCounts[targetValue] = 0;
        }
        for (int casej = firstCase; casej <= lastCase; casej++) {
            int targetValue = cases[casej][targetNode];
            targetCounts[targetValue]++;
        }
    }
	
	public double evaluateBN(BayesNet BN, Instances testData) throws Exception{
		//double AUC = 0.0;
	 ...
	 */	
		...
		public BayesNet injectTargetSpecificFeature(BayesNet bayesNet, Instances data) throws Exception{
			...
			PrintWriter printout2 = new PrintWriter(new File("00experiment/intermediate/source_clean_inject.xml"));
	        printout2.println(tempNet.graph());
	        printout2.flush(); printout2.close();
	        ConvertBIFtoGENIE conv = new ConvertBIFtoGENIE();
			conv.runner("00experiment/intermediate/source_clean_inject.xml", "", "hashCode.xml", "xdsl");
			return tempNet;
		}
    ...
    protected void deriveNodeProbs_Prior_Data(int casei, int node, double oneAdjustedPriorSampleSize) {
	  ...
	  //  	System.out.print(nodeInfo[node].name + "=" + nodeValue + "parents:" );
//	    	for (int i=1; i<=parents[node][0]; i++){
//	    		System.out.print(parentValues[i]+",");
//	    	}
//	    	System.out.println();
...
//	    	System.out.println(source_clean_BayesNet);
	...
	protected void incorporateNodeScores(int node) {
		...
		if (m_ScoreMetric==0) { //K2 is selected
			...
			a = m_PriorEquivalentSampleSize / numberOfJointStates(node);
		...
		for (int casei = firstCase; casei <= lastCase; casei++) {
			...
			for (int targetValue = 1; targetValue <= nodeDimension[targetNode]; targetValue++) {		
				...
				if (m_ScoreMetric==0 | m_ScoreMetric==1) {
					...
					deriveNodeProbs_Prior_Data(casei, node,adjustedPriorSampleSize);
				}
...
//		}
	...
	public void setPriorSampleSize(int priorSampleSize) {
		this.adjustedPriorSampleSize = priorSampleSize;
	}
/*******************Kevin artuo method **************************/
	...
	protected void initVariables(){
		...
		adjustedPriorSampleSize = getAdjustedPriorSampleSize(); //This part should be changed later.
...
/***********************************************Parking****/
	...
	//did not change parent[][]
		protected void initVariables2(){
			...
			adjustedPriorSampleSize = getAdjustedPriorSampleSize();
			maxParents = getMaxNrOfParents();
			maxChildren = getMaxNrOfChildren();
...
//	    	System.out.println(source_clean_BayesNet);
	...
	//    	System.out.println("K:" + k);	
		 ...
		 */
		//Ye added
			public BTLSM_V5(int predictors, int max_parents, int max_children, String sourceModelFile) throws Exception {
				setExpectedParentsOfTarget(predictors); 
				setMaxNrOfParents(max_parents);
				setMaxNrOfChildren(max_children);
				source_BayesNet = new BIFReader();
		 ...
		 */
		//Ye added
		public BTLSM_V5(int predictors, int max_parents, int max_children, String sourceModelFile, int sourceSize) throws Exception {
			setExpectedParentsOfTarget(predictors); 
			setMaxNrOfParents(max_parents);
			setMaxNrOfChildren(max_children);
			source_BayesNet = new BIFReader();
		 ...
		 * @throws Exception
		 */
		public BayesNet OLDinjectTargetSpecificFeature2(BayesNet bayesNet, Instances data) throws Exception{
			System.out.println("------Injecting target specific features");
			PrintWriter printout = new PrintWriter(new File("temp.xml"));
	        printout.println(bayesNet.graph());
	        printout.flush(); printout.close();
