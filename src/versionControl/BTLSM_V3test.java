...
/*****Modify from BTLSM_V1, begin adding source model, source instances, target instances. ******/
 ...
 */
public class BTLSM_V3test extends SearchAlgorithm implements TechnicalInformationHandler {
	...
	protected boolean[] childPresent; //  note: it is an array, recording whether a node has some child
...
//	private final boolean m_bInitAsNaiveBayes = false; 
	...
	protected boolean[] parentPresent; //  note: it is an array, recording whether a node has some parent
	 ...
	 * default constructor
	 */
	public BTLSM_V3test(int predictors, int max_parents, int max_children) {
	 ...
	 * default constructor
	 */
	public BTLSM_V3test(int predictors, int max_parents, int max_children, int pess) {
		setExpectedParentsOfTarget(predictors);
		setMaxNrOfParents(max_parents);
		setMaxNrOfChildren(max_children);
	 ...
	 */
	//Ye added
		public BTLSM_V3test(int predictors, int max_parents, int max_children, String sourceModelFile) throws Exception {
			setExpectedParentsOfTarget(predictors); 
			setMaxNrOfParents(max_parents);
			setMaxNrOfChildren(max_children);
			m_BayesNet = new BIFReader();
			try {
				((BIFReader) m_BayesNet).processFile(sourceModelFile);
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
			System.out.println(m_BayesNet);
			System.out.println(m_BayesNet.getParentSets().length);
		} // c'tor

		/**
		 * This method calculates Nijk' = N' * p(Xi=k, Pai=j | Bsc) by calling the getPriorProb method.
		 * @param totalNum
		 * @param node
		 * @param nodeValue
		 * @param parentValueTable
		 * @param priorNetwork
		 * @return
		 */
	    public double getPriorCount(int totalNum, int node, int nodeValue, 
	    		HashMap<Integer,Integer> parentValueTable, String priorNetwork){
	    	Double Nijk = 1.0*totalNum*getPriorProb(node,nodeValue, parentValueTable,priorNetwork);
	    	return Nijk;
	    }
	  
	    public double getPriorProb(int node, int nodeValue, HashMap<Integer,Integer> parentsValueTable, String priorNetwork){
	    	Double prob = new Double(new String("1.0"));
	    	Network net = new Network();
	    	net.readFile(priorNetwork);
	    	...
	    	nodes.add(new String(nodeInfo[node].name));	
	    	currentAssignment.add(new String(""+nodeValue));
	    	ArrayList<Integer> keys = new ArrayList<Integer>(parentsValueTable.keySet());
	    	for (int k=0; k<keys.size(); k++){
	    		nodes.add(nodeInfo[keys.get(k)].name);
	    		currentAssignment.add(new String(""+parentsValueTable.get(k)));
	    	}  			
	    	HashMap<String, String> conditions = new HashMap<String, String>();
		//Delete:	conditions.put(new String("C2055125"), new String("P"));
...
//			}	
	 ...
	 * @param targetInstances the data to work with
	 * @throws Exception if something goes wrong
	 */
	public void buildStructure(BayesNet bayesNet, Instances targetInstances) throws Exception {	
        ...
        bayesNet = m_BayesNet;
 //       m_BayesNet = bayesNet;
        m_BayesNet.m_Instances = targetInstances;		
		targetNode = targetInstances.classIndex();		
		//Source infor: numberOfSourceNode,features, parents_SBN[][], 
		
		/*Part 1. clean source model:
		 * 1.remove unobserved node
		 * 2. remove not in markov Blanket
		 * 3. move parentsToChildren*/
		
		//copySourceModel(String[][] sourceParents);				
		//doMarkovBlanketCorrection(bayesNet, instances);	
	//	m_BayesNet = processSourceModel(m_BayesNet);
			
		System.out.println("ScoreMetric: "+m_ScoreMetric); //0 is K2 method		
		
		/*Part 2. grow from source model*/
		//initiate everything, prune model.
		readCases(); // initiate numberOfNodes, nodeDimension array, numberOfCases, initiate the size of the cases[][]	
		initVariables2(); // targetNode is the last Node, did not change parent[][]
...
//		print=false;
		...
		System.out.println("final score: " + scoreNode(targetNode));
		print = false;
		System.out.println(m_BayesNet.measureBayesScore());
		System.out.println(printParents());
...
//		        //   System.out.println(m_BayesNet);
//	            }  
//	            //Below are codes that Ye added   
//	            weedoutWeakArcs();
//	            System.out.println(String.format("After weed =========== score: %f", scoreNode(targetNode)));
//	         //   System.out.println(m_BayesNet);
//	          //  m_BayesNet.estimateCPTs();
//	            PrintWriter printout = new PrintWriter(new File("model_a" + numberGraph + ".xml"));
//	            printout.println(m_BayesNet);
...
//		//}
//		updateBayesNet(targetInstances);	
//		m_BayesNet.estimateCPTs();
//		//if(m_BayesNet.getDebug() == true){	  
//			System.out.println(print(m_BayesNet));
//			print = false;
...
//			}
	...
	protected BayesNet copySourceModel(String[][] sourceParents){
		BayesNet sourceBN = new BayesNet();
		for(int i = 1; i < parents.length-1;i++){
			...
			if(parents[i][0] > 0){
				for(int j = 1; j<=parents[i][0]; j++){
					...
					ParentSet currentSet = sourceBN.getParentSet(i-1);
					...
					if (!currentSet.contains(iNode)){
						currentSet.addParent(iNode, sourceBN.m_Instances);
					}
				}
			}
			//System.out.println();
	...
	public void updateBayesNet(Instances targetInstances) throws Exception{
		...
		for(int iNode = 0; iNode <= nNode-1;iNode++){	
			...
			//	Arrays.sort(list, Collections.reverseOrder());		
				...
				for (int j=0; j<=list.length-1; j++){		
					...
					if (!parentPresent[theParent]) {parentPresent[theParent]=true;}
					if (!childPresent[theChild]) {childPresent[theChild]=true;}
	...
	//Unfinished Ye method: use m_BayesNet to update parents[][], parentPresent[], childPresent[], 
	//numberOfChildren, children[], map[]
	//This method is similar as updateParents method, but it adds child of the target node in order 
	//(adding node that are parents of other nodes first)
	//m_ParentSets[iNode].getParent(iParent);
	public void updateParentsWithOrder(){
		parents = new int[numberOfNodes + 1][maxParents + 1];
		int nNode = m_BayesNet.getNrOfNodes();
		System.out.println("number of Nodes" + nNode);	
		for(int iNode = 0; iNode <= nNode-1;iNode++){		
			...
			//	Arrays.sort(list, Collections.reverseOrder());		
				...
				for (int j=0; j<=list.length-1; j++){
					...
					if (theParent==targetNode) {
						...
						children[numberOfChildren] = theChild;
						map[theChild]=numberOfChildren;
						incorporateNodeScores(theChild); //Ye added to get lnChildrenProb[][] ready for scoring the initial model
					}
				//	System.out.println(parents[iNode+1][j+1]);
				}
			}
   ...
   //     parents[node][0] = 0;
     ...
     */
    protected boolean moveParentsToChildren() {
        sortTargetParents(); //descending sort parents. for example node 100 will be the first parent. node 2 will be the last.
    ...
    protected String printParents(){
    	String model = "";
    	int numberOfNode = parents.length-1;
		//Initialize BayesNet to single variable
