...
/*****
 * BTLSM_V3 is initiated with a structure. The CPT of the structure has not been used for scoring and 
 * final CPT calculation. The bn is still an empty BN. The search algorithm does not return a BN.
 * The scoring part is still the original EBMC: marginal likelihood with K2 or BDeu. structure prior.
 * The numeric parameters matter a lot for structure finding. Be careful about definding them. 
 * ******/
 ...
 */
public class BTLSM_V3 extends SearchAlgorithm implements TechnicalInformationHandler {
...
//	private final boolean m_bInitAsNaiveBayes = false; 
	 ...
	 * default constructor
	 */
	public BTLSM_V3(int predictors, int max_parents, int max_children) {
	 ...
	 * default constructor
	 */
	public BTLSM_V3(int predictors, int max_parents, int max_children, int pess) {
		setExpectedParentsOfTarget(predictors);
		setMaxNrOfParents(max_parents);
		setMaxNrOfChildren(max_children);
	 ...
	 */
	//Ye added
		public BTLSM_V3(int predictors, int max_parents, int max_children, String sourceModelFile) throws Exception {
			setExpectedParentsOfTarget(predictors); 
			setMaxNrOfParents(max_parents);
			setMaxNrOfChildren(max_children);
	 ...
	 */	
		 ...
		 * @return
		 */
	    public double getPriorCount(int equivalentSampleSize, int node, int nodeValue, 
	    		HashMap<Integer,Integer> parentValueTable, String priorNetwork){
	    	Double Nijk = 1.0*equivalentSampleSize*getPriorProb(node,nodeValue, parentValueTable,priorNetwork);
	    	return Nijk;
	    }
