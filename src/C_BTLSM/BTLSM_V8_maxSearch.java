 ...
 */
public class BTLSM_V8_maxSearch extends SearchAlgorithm implements TechnicalInformationHandler {
	
	static int maxSearch = 1; //This parameter decides how many parents the search will find in one grow
	...
	sourceCleanInjectModelXDSL,sourceSimulateDataLoc,sourceSimulateDataName,sourceDataLoc,sourceDataName,sourceDataSize,
...
//	private final boolean m_bInitAsNaiveBayes = false; 
	...
	//1 is sample size ratio; 2 is KL; 3 is Bayes factor
	 ...
	 * default constructor
	 */
	public BTLSM_V8_maxSearch() {
	} // c'tor

	/**
	 * default constructor
	 */
	public BTLSM_V8_maxSearch(int predictors, int max_parents, int max_children) {
	 ...
	 * default constructor
	 */
	public BTLSM_V8_maxSearch(int predictors, int max_parents, int max_children, int pess) {
		setExpectedParentsOfTarget(predictors);
		setMaxNrOfParents(max_parents);
		setMaxNrOfChildren(max_children);
	 ...
	 */
	//Ye added
	public BTLSM_V8_maxSearch(int maxParent, String configFile, String scoreMethod, String adjustMethod, 
	 ...
	 */
	//Ye added
	public BTLSM_V8_maxSearch(String configFile, int nSearch, String scoreMethod, String adjustMethod, 
			String startNetwork,  double trueKL1, HashMap<String,Double> trueBFWeightTable1,
			double avgKL1, HashMap<String,Double> avgBFWeightTable1) throws Exception {
		maxSearch = nSearch;
		trueKL = trueKL1;
	...
	public void buildStructure(BayesNet bayesNet, Instances targetTrainingData) throws Exception {	
		 ...
		 * 4. inject target specific feature.
...
//	 	net.readFile("00experiment/intermediate/source_clean_inject.xdsl");
		net.readFile(sourceModelLoc+sourceCleanInjectModelXDSL);
...
//    		String[] keyList = weightTable.keySet().toArray(new String[0]);
...
//		PrintWriter printout = new PrintWriter(new File("00experiment/learnedModel/final.xml"));
		...
		protected double scoreNode(int node) {
			...
			for (int casei = firstCase; casei <= lastCase; casei++) {			
				...
				//	System.out.println("derive node prob using prior model and target data");
					...
					if (m_AdjustSourceSampleSizeMetric == 3){																
	...
	public void setConfig() throws FileNotFoundException{
		...
		sourceCleanInjectModelXDSL=Utility.getConfig("sourceCleanInjectModelXDSL",configFileName);
...
//				KL = Utility.getTrueAverageKL(sourceDataSize, sourceModelLoc, sourceTrueModelName, 
//						targetModelLoc, targetTrueModelName, sourceSimulateDataLoc) ;
				KL = trueKL ;
			}
			else {
//				KL = Utility.getAverageKL(targetDataLoc+targetDataName,
//					sourceDataSize,sourceModelLoc,sourceLearnedCleanedModelName,sourceSimulateDataLoc, 
...
//			nodeList.add("TI"); nodeList.add("G"); nodeList.add("class"); 
			if (useTrueSourceDataForWeight){
	    ...
	    //	System.out.println("m_PriorEquivalentSampleSize "+m_PriorEquivalentSampleSize);
	    	...
	    	//		printList(parentSetInCandiate);
	    	 ...
	    	 //		System.out.println("difference " + difference(parentSetInSource, parentSetInCandiate));
	    	 		...
	    	 		if (m_AdjustSourceSampleSizeMetric == 3 ) // for bayes factor
	    	 				//if it is shared feature, then pesudocount is weighted by bayes factor
	    	 			{
	    	 				if (sharedNodeList.contains(iNodeName)) {
...
/***********************************************Parking****/
	...
	public void searchParents(int node) {
        ...
        System.out.println("maxParents is: " + maxParents);
        while (parents[node][0] < maxParents) {
            bestScore = 1;
            bestParent = 0;
            allTrue = true;
...
//	    	System.out.println(source_clean_BayesNet);
	...
	//    	System.out.println("K:" + k);	
		 ...
		 */
		//Ye added
			public BTLSM_V8_maxSearch(int predictors, int max_parents, int max_children, String sourceModelFile) throws Exception {
				setExpectedParentsOfTarget(predictors); 
				setMaxNrOfParents(max_parents);
				setMaxNrOfChildren(max_children);
				source_BayesNet = new BIFReader();
		 ...
		 */
		//Ye added
		public BTLSM_V8_maxSearch(int predictors, int max_parents, int max_children, String sourceModelFile, int sourceSize) throws Exception {
			setExpectedParentsOfTarget(predictors); 
			setMaxNrOfParents(max_parents);
			setMaxNrOfChildren(max_children);
			source_BayesNet = new BIFReader();
