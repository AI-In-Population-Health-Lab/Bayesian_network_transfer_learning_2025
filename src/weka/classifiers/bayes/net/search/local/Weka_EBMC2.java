 ...
 */
public class Weka_EBMC2 extends SearchAlgorithm implements TechnicalInformationHandler {
	 ...
	 * default constructor
	 */
	public Weka_EBMC2(int predictors, int max_parents, int max_children) {
	 ...
	 * default constructor
	 */
	public Weka_EBMC2(int predictors, int max_parents, int max_children, int pess) {
		setExpectedParentsOfTarget(predictors);
		setMaxNrOfParents(max_parents);
		setMaxNrOfChildren(max_children);
	...
	protected void deriveNodeProbs(int casei, int node, double k, double v) {
		...
		//ptr: Temporary pointer that leads the path of the countsTree to arrive the final point of countsTree
		//ctPtr: countsTree pointer to get value from countsTree array
		//cPtr: count pointer to get value from counts array
		
		numberOfParents = parents[node][0];
		for (int i = 1; i <= numberOfParents; i++)
			values[i] = cases[casei][parents[node][i]]; //Ye note: values of parents of the node
		ctPtr = 1;
		for (int i = 1; i <= numberOfParents; i++) {
